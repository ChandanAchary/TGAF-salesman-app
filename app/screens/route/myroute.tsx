...
export default function MyRouteScreen() {
  ...
  const [selectedMapCustomer, setSelectedMapCustomer] = useState<RouteCustomer | null>(null);
  ...
  const customersWithDistance = useMemo(() => {
    ...
    if (!locationTracking || !userLocation) {
      setOptimizingRoute(false);
      return routeQuery.data.data.customers.map((customer) => ({
        ...customer,
        distance: null as number | null,
      }));
    }
    const optimizedCustomers = routeQuery.data.data.customers
      .map((customer) => ({
        ...customer,
  ...
  const completionPercentage = useMemo(() => {
    if (!customersWithDistance.length) return 0;
    return Math.round((visitedCustomerIds.size / customersWithDistance.length) * 100);
  }, [customersWithDistance, visitedCustomerIds]);

  // Loading state
  if (routeQuery.isLoading) {
    return (
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        <TabBar title="Today's Route" />
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={Theme.colors.primary} />
          <Text style={styles.loadingText}>Loading your route plan...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  // No route assigned
  if (!routeQuery.data?.data.route) {
    return (
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        <TabBar title="Today's Route" />
        <View style={styles.emptyContainer}>
          <LinearGradient
            colors={[Theme.colors.primaryLight, 'rgba(99,102,241,0.05)']}
          ...
          <Text style={styles.emptyText}>
            You don't have any route assigned for today. Please check with your
            manager or try again later.
          </Text>
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={styles.emptyActionButtonText}>Refresh Beat</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
        ...
        <View style={styles.mapContainer}>
          ...
          >
            {filteredCustomers.map((customer) => {
              ...
              // Pin colors matching plan: green for visited, yellow for ordered, red for pending
          ...
          {/* Map Selected Customer Info Card (Bottom Sheet style overlay) */}
          {selectedMapCustomer && (
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    paddingBottom: 110, // Generous padding to clear floating nav
  },
  dashboardCard: {
    margin: 16,
