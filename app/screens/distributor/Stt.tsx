...
export default function Stt() {
  ...
  const sttQuery = useQuery({
    queryKey: ["stt"],
    queryFn: async () => {
      const res = await api.get<SttResponse>(API_ROUTES.CITY_HEAD.STT.GET_TODAY(distributorId as string));
      return res.data;
    },
  });
  ...
  const closingStockMuation = useMutation({
    mutationFn: async (data: UpdateClosingStockParams) => {
      const res = await api.post(
        API_ROUTES.CITY_HEAD.ORDER.UPDATE_STOCK,
        data
      );
      return res.data;
    },
  ...
  const calculateStockSummary = (): StockSummary[] => {
    if (!productQuery.data?.data || !sttQuery.data?.data) return [];

    return productQuery.data.data.map(product => {
      const sttItem = sttQuery.data.data?.STTItem.find(item => item.productId === product.id);
